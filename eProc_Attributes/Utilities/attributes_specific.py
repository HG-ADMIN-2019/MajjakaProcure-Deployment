...
def save_attr_level_data(current_attr_list, inherited_list, attr_object_id):
    created = ''
    ...
    for count, current_attr in enumerate(current_attr_list):
        ...
        if (current_attr not in inherited_list) or (exclude_consider_flag == True) or (default_consider_flag == True):
            ...
            if current_attr['attr_level_exclude']:
                ...
                exclude_flag = False
            if OrgAttributesLevel.objects.filter(client=global_variables.GLOBAL_CLIENT,
                                                 attribute_id=current_attr['attribute_id'],
                                                 low=current_attr['value'],
                                                 object_id=attr_object_id).exists():
                if not OrgAttributesLevel.objects.filter(client=global_variables.GLOBAL_CLIENT,
                                                         attribute_id=current_attr['attribute_id'],
                                                         low=current_attr['value'],
                                                         attr_level_exclude=True,
                                                         object_id=attr_object_id).exists():
                    if default_flag:
                        OrgAttributesLevel.objects.filter(client=global_variables.GLOBAL_CLIENT,
                                                          attribute_id=current_attr['attribute_id'],
                                                          low=current_attr['value'],
...
def save_attr_id_data_into_db(attr_id_data, attr_object_id):
    """
    ...
    save_node_level_attr = save_attr_level_data(current_attr_list, inherited_list, attr_object_id)
    # save exclude in node level
    created = save_attr_level_data(inherited_exclude_save, empty_array, attr_object_id)
