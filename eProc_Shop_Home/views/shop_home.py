...
from eProc_Basic.Utilities.messages.messages import MSG158, MSG159, MSG160, MSG113, MSG112
...
def shopping_cart_home(request):
    ...
    support_email = django_query_instance.django_get_query(OrgSupport, {
        'org_support_types': 'EMAIL', 'client': client, 'del_ind': False
    })

    if support_email is not None:
        support_email = support_email.org_support_email

    support_phone = django_query_instance.django_get_query(OrgSupport, {
        'org_support_types': 'CALL', 'client': client, 'del_ind': False
    })

    if support_phone is not None:
        support_phone = support_phone.org_support_number
...
def add_favourite_shopping_cart(request):
    ...
    if request.method == 'POST':
        ...
        if len(fav_cart_list_count) == CONST_FAVOURITE_SC_LIMIT:
            return JsonResponse({'error_message': MSG158}, status=201)
        else:
            ...
            if django_query_instance.django_existence_check(FavouriteCart, {
                ...
                return JsonResponse({'error_message': MSG159}, status=201)
            ...
            return JsonResponse({'success_message': MSG160}, status=201)
