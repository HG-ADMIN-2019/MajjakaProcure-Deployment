   ...
   let opt_selected = sel.options[sel.selectedIndex];
    ...
    if ( !AddNodeResult.hasOwnProperty('error') )
    {
        ...
        $("#modalAddNode").modal('hide');
          if( new_node_arrow_class == "fa fa-minus-circle")
          {
               ...
               document.getElementById(new_node_append_ul).appendChild(lis);
                ...
                var node_t = AddNodeResult[0].fields.node_type
                 if (node_t != 'USER') {
                    ...
                    iElement.id = liId + "i";
                    iElement.className = "fa fa-plus-circle";
...
{
    $("#a_un_users_table").empty();
    let table_head = " <thead><tr><th>Assign/Un Assign</th><th>Employee No.</th><th>Employee name</th><th>Employee Email</th></tr></thead>"
    $("#a_un_users_table").append(table_head)
    ...
    for (let i = 0; i < get_users_list.length; i++)
    {
        ...
        user_td = '<tr class="users_assign"><td ><input onclick= "assign_unassign(this);"  type="checkbox" id = '+ user_fname +' ></td><td>'+ user_id +'</td><td>'+ user_fname +'</td><td>'+ user_email +'</td></tr>'

        $("#a_un_users_table").append(user_td)

     }
     tableSortFilter('myTableData')
}
...
function save_assign_unassign_status()
{
    ...
    GLOBAL_ONCLICK_NODE_GUID = NodeGuidByMapId(jsonData, delete_element_value);
     ...
     {
         alert(messageConstants.JMSG0011)
     ...
     else
    {
        ...
        console.log(result)
        if (a_un_arrow == "fa fa-minus-circle") {
