        ...
        try:
            if guid is not None:
                ...
                if parent_node_guid.count() != 0:
                    return guid
                self.set_error(MSG143)
        except ValidationError:
            self.set_error(MSG144)
        return None
        ...
        if self.alpha_num(object_id):
            if not self.check_map_id_exist(object_id):
                if len(object_id) <= 8:
                    ...
                    self.set_error(MSG145)
            else:
                self.set_error(MSG146)
        else:
            self.set_error(MSG147)
        return None
