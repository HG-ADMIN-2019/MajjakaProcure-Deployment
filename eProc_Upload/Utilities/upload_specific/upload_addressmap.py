    ...
    try:
        ...
        if Test_mode == 'on':
            ...
            for column in csv.reader ( addressmap, delimiter=',', quotechar='"' ):
                ...
                if (del_ind_field == '0') and (OrgAddressMap.objects.filter ( address_number=column[1], client=client ).exists()):
                ...
                # If the Input file Deletion indicator is "0" (Insert\Update record)
                    ...
                    if column[1] in Test_addressmap_number_list:
                        Test_Duplicate_Count_addressmap = Test_Duplicate_Count_addressmap + 1
                        Test_addressmap_errmsg_duplicate = MSG094
                ...
                if (del_ind_field == '1') and (OrgAddressMap.objects.filter ( address_number=column[1], client=client ).exists()):
            ...
            for column in csv.reader ( addressmap, delimiter=',', quotechar='"' ):
                ...
                if (del_ind_field == '0') and (not (OrgAddressMap.objects.filter ( address_number=column[1], client=client ).exists ())):
                ...
                elif (del_ind_field == '0') and (OrgAddressMap.objects.filter ( address_number=column[1], client=client ).exists ()):
                ...
                elif (del_ind_field == '1') and (not (OrgAddressMap.objects.filter ( address_number=column[1], client=client).exists ())):
