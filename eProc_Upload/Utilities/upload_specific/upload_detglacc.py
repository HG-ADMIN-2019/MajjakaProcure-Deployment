    ...
    try:
        ...
        if Test_mode == 'on':
            ...
            for column in csv.reader ( detglacc, delimiter=',', quotechar='"' ):
                ...
                if (del_ind_field == '0') and (DetermineGLAccount.objects.filter ( gl_acc_num=column[2], client=client ).exists()):
                    ...
                    for test in queryset_test_upd:
                        if (test.from_value != column[0] or test.to_value!=column[1] or test.default!=column[3]
                            or test.company_id!=column[4] or test.del_ind!=column[5] or test.account_assign_cat!=column[6]
                ...
                # If the Input file Deletion indicator is "0" (Insert\Update record)
                    ...
                    if column[3] in Test_detglacc_number_list:
                        Test_Duplicate_Count_detglacc = Test_Duplicate_Count_detglacc + 1
                        Test_detglacc_errmsg_duplicate = MSG094
                ...
                if (del_ind_field == '1') and (DetermineGLAccount.objects.filter ( gl_acc_num=column[2], client=client ).exists()):
            ...
            for column in csv.reader ( detglacc, delimiter=',', quotechar='"' ):
                ...
                if (del_ind_field == '0') and (not (DetermineGLAccount.objects.filter ( gl_acc_num=column[2], client=client ).exists ())):
                ...
                elif (del_ind_field == '0') and (DetermineGLAccount.objects.filter ( gl_acc_num=column[2], client=client ).exists ()):
                ...
                elif (del_ind_field == '1') and (not (DetermineGLAccount.objects.filter ( gl_acc_num=column[2], client=client).exists ())):
