...
from eProc_Configuration.models import EformFieldConfig, ProductEformPricing, ProductInfo, ProductsDetail, Catalogs, \
    OrgClients, CatalogMapping, FreeTextDetails
...
def save_product_details_eform(eform_configured, product_id):
    """
...
def save_product_specification(product_specification_data, product_id):
    ...
    if ProductsDetail.objects.filter(Q(product_id=product_id) & ~Q(product_info_id=None)):
        product_info_id = DjangoQueries().django_filter_value_list_ordered_by_distinct_query(ProductsDetail,
                                                                                             {'product_id': product_id,
                                                                                              'client': global_variables.GLOBAL_CLIENT,
                                                                                              'del_ind': False},
                                                                                             'product_info_id',
...
def product_eform_price_query_dictionary_list(eform_configured, eform_id):
    ...
    for eform_data in eform_configured:
        ...
        eform_field_dictionary = {'pk': eform_guid,
                                  ...
                                  'eform_id': eform_id,
                                  'eform_field_count': eform_field_count,
            ...
            if eform_data['field_type'] != CONST_VARIANT_WITHOUT_PRICING:
                if eform_data['field_type'] != CONST_QUANTITY_BASED_DISCOUNT:
                    if variant_data['default_option']:
                        eform_field_dictionary['default_eform_field_data'] = variant_data['option_value']
        ...
        eform_field_dictionary['eform_field_data'] = eform_field_data
